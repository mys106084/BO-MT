...
class Alignment(object):
    ...
    def UpdateCounts(self):
        ...
        for s in xrange(0,self.sum_s):
            ...
            for i in xrange(0,m):
                ...
                for j in xrange(0,l):
                    self.prob.append(self.GetT(self.sentences_f[s][i],self.sentences_e[s][j])*self.GetQ(j,i,l,m,z)*self.normprob)
                    normalization += self.prob[j]
                self.prob.append(self.GetT(self.sentences_f[s][i],-1)*self.nullprob)
                norm_noNA = normalization
                normalization += self.prob[l]
                # Print alignments
                for j in xrange(0,l):
                    self.delta[j] = self.prob[j]/normalization

                    #self.emp_feat += self.prob[j]/norm_noNA*Feature(i+1,j+1,l,m)

                #nullAlignment
                self.delta[-1] = self.prob[l]/normalization
         #---------------------------------------UpdateCounts---------------------------------------
                for j in xrange(0,l):
                    # Count C(e,f)
                    self.count_fe[(self.sentences_f[s][i],self.sentences_e[s][j])] += self.delta[j]
                    # Count C(e)
                    self.count_e[self.sentences_e[s][j]] += self.delta[j]

                    self.emp_feat += self.delta[j]*Feature(i+1,j+1,l,m)
                # nullAlignment
                self.count_fe[(self.sentences_f[s][i],-1)] += self.delta[-1]
                self.count_e[-1] += self.delta[-1]
    ...
    def UpdateCounts_IBM1(self):
        ...
        for s in xrange(0,self.sum_s):
            if s%10000 == 0:
                print "E-step- ComputeDelta - Sentence:"+str(s)
                '''
                print 'Size of sentences_e:'+str(sys.getsizeof(self.sentences_e))
                print 'Size of sentences_f:'+str(sys.getsizeof(self.sentences_f))
                print 'Size of wordmap_e:'+str(sys.getsizeof(self.wordmap_e))
                print 'Size of wordmap_f:'+str(sys.getsizeof(self.wordmap_f))
                print 'Size of words_e:'+str(sys.getsizeof(self.words_e))
                print 'Size of words_f:'+str(sys.getsizeof(self.words_f))
                '''
                print 'Size of prob:'+str(sys.getsizeof(self.prob))
                print 'Size of delta:'+str(sys.getsizeof(self.delta))
                print 'Size of cout_e:'+str(sys.getsizeof(self.count_e))
                print 'Size of cout_fe:'+str(sys.getsizeof(self.count_fe))
                print 'Size of T probability:'+str(sys.getsizeof(self.t))
            m = len(self.sentences_f[s])
            l = len(self.sentences_e[s])
            for i in xrange(0,m):
                self.delta.clear()
                ...
                normalization = 0
                for j in xrange(0,l):
                    self.prob.append(self.GetT(self.sentences_f[s][i],self.sentences_e[s][j])*self.normprob)
                    normalization += self.prob[j]
                self.prob.append(self.GetT(self.sentences_f[s][i],-1)*self.nullprob)
                normalization += self.prob[l]
                for j in xrange(0,l):
    ...
    def DEV(self):
        ...
        for s in xrange(0,len(self.sentences_dev_e)):
            m = len(self.sentences_dev_f[s])
            l = len(self.sentences_dev_e[s])
